		...
		if (tpd_touchinfo(&cinfo, &pinfo))
		{
			...
			if(point_num >0) 
			{
				...
				while(i<point_num)
				{
                    ...
                    //printk("[mtk-tpd] Original position/size: [%4d %4d %4d]\n", input_x, input_y, 10);
					tpd_calibrate_driver(&input_x, &input_y);
                    //printk("[mtk-tpd] cabration position/size: [%4d %4d %4d]\n", input_x, input_y, 10);
                                           temp = input_x;
                                           input_x = input_y;
                                           input_y = temp;
                                           input_y = 768-input_y;
...
{	 
    ...
    mt_set_gpio_mode(GPIO142, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO142, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO142, GPIO_OUT_ONE);
    mdelay(30);
        ...
        for(i = 0; i < 2; i++) /*Do Power on again to avoid tp bug*/
	{
		...
		mdelay(50);
    		...
    		//hwPowerOn(TPD_POWER_SOURCE, VOL_1800, "TP");
#endif
#ifdef TPD_POWER_SOURCE_1800
